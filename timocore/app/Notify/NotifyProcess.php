...
class NotifyProcess
{
    ...
    */
     ...
     * @return string|bool
     */
    protected function getMessage($randomTrx = null)
    {
        ...
        if ($user && $template) {
            $message = $this->replaceShortCode($user->fullname, $user->fullname, gs($globalTemplate), $template->$body);
            if (empty($message)) {
     ...
     * @return void
     */
    public function createLog($type, $randomTrx = null)
    {
        ...
        if ($this->user && $this->createLog) {
            ...
            $notificationLog->message = $this->finalMessage;
