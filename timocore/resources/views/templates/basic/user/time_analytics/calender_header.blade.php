<div class="table-filter-left">
    <div class="datepicker-arrow">
        <button id="btn-prev" class="datepicker-arrow-btn" type="button" data-y="{{ $prev->year }}"
            data-m="{{ $prev->month }}">
            <i class="fa-solid fa-arrow-left-long"></i>
        </button>
        <button id="btn-next" class="datepicker-arrow-btn" type="button" data-y="{{ $next->year }}"
            data-m="{{ $next->month }}" @disabled(date('Ym') < $next->year.$next->month) data-current="{{ date('Ym') }}">
            <i class="fa-solid fa-arrow-right-long"></i>
        </button>
    </div>
    <h6 id="month-label" class="mb-0" data-y="{{ $year }}" data-m="{{ $month }}">{{ $monthLabel }}
    </h6>
</div>
